import { debug, error, log } from 'cc';
    ...
    public async run(onProgress?: (progress: number) => void): Promise<void> {
        ...
        for (const group of this.groups) {
            if (Array.isArray(group)) {
                // =============== 並行執行 (Parallel) ===============
                debug('TaskChain', `[Parallel Group] Starting ${group.length} tasks concurrently.`);

                const promises = group.map(async (item) => {
                    if (item.description) {
                        debug('TaskChain', `  -> Parallel Task: ${item.description}`);
                    }
                    try {
                        await item.task();
                    } catch (err) {
                        error('TaskChain', `Parallel Task failed: ${item.description}`, err);
                        throw err;
                    }
                    // 單個平行任務完成後，推進整體權重
                ...
                if (group.description) {
                    debug('TaskChain', `[Sequential Task] Running: ${group.description}`);
                }

                try {
                    await group.task();
                } catch (err) {
                    error('TaskChain', `Task failed: ${group.description}`, err);
                    throw err;
                }
        ...
        log('TaskChain', 'All tasks completed successfully.');
    }
}
