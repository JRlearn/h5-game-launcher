...
為了確保代碼的可維護性與團隊協作效率，請遵循以下規範：

*   **JSDoc 註釋**: 所有類別、成員成員（屬性）及方法必須提供 JSDoc 註釋。
    *   **語言要求**: 註釋內容必須使用**繁體中文**。
    *   **格式要求**: 必須包含方法描述，並使用 `@param`、`@returns`（若有）等標籤。
    *   *範例*：
        ```typescript
        /**
         * 根據用戶 ID 獲取玩家頭像
         * @param userId 用戶唯一標識
         * @returns 返回頭像的 URL 字符串
         */
        public getAvatarByUserId(userId: string): string { ... }
        ```
*   **複雜邏輯說明**:
    *   在處理複雜演算法、狀態機管理、或異步網絡請求的代碼塊中，必須補上細緻的行內註釋 (`//`) 說明實作邏輯與邊界條件。
*   **命名規範**:
    *   **類別/接口/枚舉**: 使用 `PascalCase`（如 `LoginController`）。
    *   **變量/方法**: 使用 `camelCase`（如 `updatePlayerInfo`）。
    *   **私有屬性**: 建議加下劃線前綴（如 `_isInitialized`）。
    *   **常量**: 使用 `UPPER_SNAKE_CASE`（如 `MAX_RETRY_COUNT`）。
*   **類型安全 (Type Safety)**:
    *   **嚴禁滥用 `any`**: 除非極端特殊情況，否則必須定義明確的類型、接口或使用泛型。
    *   **明確返回類型**: 每個方法都必須明確標註返回類型（如 `void`, `Promise<boolean>`）。
*   **異步編程**:
    *   優先使用 `async/await` 處理異步邏輯，避免過深的 Promise 鏈或回調地獄。
*   **Cocos 性能優化**:
    *   在 `update` 生命週期內避免頻繁的 `getComponent` 調用，應在 `onLoad` 或 `start` 中預先緩存。
